...
int main(int argc, char *argv[])
{
    ...
    // read in the user-specified verbosity level
	verbose = atoi(argv[1]);
    ...
    while (read_graph(&n, degree, G) == 0)
    {
		...
		if (verbose == 1)
		{
			...
			min_dom_set(0, delta, n, degree, G, &n_dominated, num_dominated, num_choice, &size, dom, &min_size, min_dom);
...
int read_graph(int* n, int degree[NMAX], int G[NMAX][DEG_MAX]) 
{
    // read in input as long as end of file is not reached
				 ...
				 int num_choice[NMAX], int* size, int dom[NMAX], int* min_size, int min_dom[NMAX])
{
	...
	// recursive call
	min_dom_set(level + 1, delta, n, degree, G, n_dominated, num_dominated, num_choice, size, dom, min_size, min_dom);
